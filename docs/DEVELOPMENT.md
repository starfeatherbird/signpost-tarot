...
- 흐름
  - 무료: 상품 선택 → 고민 입력 → 상황 확인 3문항(원하는 도움 · 지키고 싶은 것 · 어려운 조건) → 카드 3장 → 기본 결과
  - "원하는 도움"(`src/config/helpModes.ts`: 마음을 먼저 정리하기 / 선택지를 비교하기 / 오늘 할 일 정하기)은 일반 답변으로 저장·전송되지만, 서버 프롬프트(counsel-v5)가 이 답만 따로 읽어 결과의 무게를 바꿉니다. 결과 상단에 태그로 표시. 건너뛰면 기본 균형.
...
- 하단 탭 아이콘은 인라인 SVG 선 아이콘(`BottomNav.tsx`: 카드 세 장 / 기록장 책 / 표지판), 글꼴은 Pretendard(jsdelivr) + Cinzel(카드명만) + Gowun Batang(제목).
- 첫 화면 장면 `HeroScene.tsx`: 금빛 후광 + 표지판 심볼 + 카드 뒷면 세 장 부채꼴(기존 back.webp 재사용, 그림 파일 없음).
- **분위기 강화(2026-09-12, 시안 위에 덧씌움)**: 금색 보조 토큰(`--color-gold*`, 태그 번호·섹션 라벨·카드 테두리 광택·선택 글로우), 배경 깊이(`--bg-scene` 방사형 + `body::before` 별점, 밝은 테마는 별점 없음), 제목 세리프(`--font-display` = Gowun Batang: 화면·패널·질문 제목, 결과 카드 라벨), 카드 선택 그리드의 미세 기울기와 금색 글로우, 공개 시 금빛 번짐(`reveal-glow`), 결과 상단 세 장을 스프레드 천 위에 크게. 버튼·링크 색(#1D559C)과 본문 글꼴은 그대로. 동작 줄이기 설정에서는 애니메이션 없음.
